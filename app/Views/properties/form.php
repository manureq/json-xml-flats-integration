...
<section class="panel">
    <div class="panel__body">
        <form method="post" action="<?= $property ? '/properties/update' : '/properties'; ?>" class="form-grid form-grid--responsive">
            <?= csrf_field(); ?>
            <?php if ($property): ?>
            ...
            <div class="form-section">
                <h3>Descripción</h3>
                <label class="full">Información destacada
                    <textarea name="description" rows="6" placeholder="Incluye diferenciales, condiciones y puntos fuertes."><?= htmlspecialchars($property['description'] ?? '', ENT_QUOTES, 'UTF-8'); ?></textarea>
                </label>
            </div>

            <div class="form-section">
                <h3>Amenities</h3>
                <div class="repeatable" id="amenities">
                    <?php $existingAmenities = $amenities ?? ($property['amenities'] ?? []); ?>
                    <?php if ($existingAmenities === []): ?>
                        <div class="repeatable__group">
            ...
            <div class="form-section">
                <h3>Multimedia</h3>
                <div class="repeatable" id="media-items">
                    <?php $existingMedia = $media ?? ($property['media'] ?? []); ?>
                    <?php if ($existingMedia === []): ?>
                        <div class="repeatable__group repeatable__group--media">
            ...
            <div class="form-actions">
                <button class="button button--primary" type="submit">Guardar propiedad</button>
            </div>
        </form>
    </div>
</section>
